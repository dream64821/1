...
{#if browsingCollection}
	...
	{:else}
		...
		{#each allBrowsingItems as item, idx}
			...
			{#if item._itemType === 'directory'}
				<div
					class="px-2 py-1 rounded-xl w-full flex items-center {idx === selectedIdx
						? 'bg-gray-50 dark:bg-gray-800 dark:text-gray-100 selected-command-option-button'
						: ''}"
				>
					<button
						type="button"
						class="flex-1 text-left flex items-center gap-1 text-black dark:text-gray-100"
			...
			{:else}
				<button
					class="px-2 py-1 rounded-xl w-full text-left flex items-center gap-1 text-black dark:text-gray-100 {idx ===
					selectedIdx
						? 'bg-gray-50 dark:bg-gray-800 dark:text-gray-100 selected-command-option-button'
						: ''}"
					type="button"
	...
	{#each filteredItems as item, idx}
		...
		{#if !['youtube', 'web'].includes(item.type)}
			{#if item.type === 'collection'}
				<!-- Collection row: click to browse in, plus button to select entire collection -->
				<div
					class="px-2 py-1 rounded-xl w-full flex items-center {idx === selectedIdx
						? ' bg-gray-50 dark:bg-gray-800 dark:text-gray-100 selected-command-option-button'
						: ''}"
			...
			{:else}
				<!-- Folder or File row: existing behavior -->
				<button
